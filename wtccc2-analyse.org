...
#+begin_src python
    def __init__(self):
        ...
        op.add_option('--exclude', dest='excludefile', type='string', default=None,
                      help='File containing IDs of individuals to exclude in addition to project exclusions (one ID per line)')
...
#+begin_src python
    def create_data_set(self):
        ...
        print('Intersecting and concatenating chromosome files\n')
        
        if not all(map(os.path.exists, [coh + '.gen' for coh in self.cohorts])):
        ...
        if not (files_exist(rfiles) or files_exist(xfiles)):
            self.concatenate_and_subset_snps_and_convert_to_geno()
...
***** Subset snps and convert to geno
      - In each cohort, concatenate across chromosomes and create a
        .geno file at the requested SNPs
      - Remove per-chromosome and genome-wide cohort files
#+begin_src python
    def concatenate_and_subset_snps_and_convert_to_geno(self):
        for coh in self.cohorts:
            ...
            cmd = 'shellfish --make-geno --file %s --file2 %s --out %s' % \
                (coh, self.snpfile, restricted_genofile(coh, self.snpfile) )
            print(cmd)
            os.system(cmd)
            system('rm %s.gen %s.map' % (coh,coh))
            system('rmdir %s' % __insectdir__)
#+end_src
***** Check map files are identical
#+begin_src python
    def reduce_to_single_map_file(self):
        map_files = [restricted_genofile(coh, self.snpfile) + '.map' for coh in self.cohorts]
        diffs = [diff(map_files[i], map_files[i-1]) for i in range(1, len(map_files))]
        if any(diffs):
            raise Exception('Map files not identical')
#+end_src
...
def restricted_genofile(coh, snpfile):
    return coh + '-' + os.path.basename(snpfile)
#+end_src
*** Exclude dir
#+begin_src python
def exclude_dir(coh):
...
def excluded_genofile(coh, snpfile):
    return coh + 'x-' + os.path.basename(snpfile)
#+end_src
